...
| Rule | Code |
| --- | --- |
| Real multi-tenancy on one Postgres DB | `prisma/migrations/20260914100000_init/migration.sql` (RLS + `FORCE ROW LEVEL SECURITY`) |
...
| Clear cart is separate destructive DELETE | `DELETE /cart` (not invoked by switch) |
...
| Null barcode → unavailable | `productMatch.ts` |
...
| DELETE | `/cart` | `src/routes/cart.ts` |
